...
//something.toCharArray(letters,something.length()+1,0);
...
char msg[] = "AB"; //Change the text here.


//Columns
int colClockPin = PB1; //Arduino pin connected to Clock Pin 11 of 74HC595
int colLatchPin = PB0; //Arduino pin connected to Latch Pin 12 of 74HC595
int colDataPin = PA7;  //Arduino pin connected to Data Pin 14 of 74HC595

//Rows
int rowClockPin = PA6; //Arduino pin connected to Clock Pin 11 of 74HC595
int rowLatchPin = PA5; //Arduino pin connected to Latch Pin 12 of 74HC595
int rowDataPin = PA4;  //Arduino pin connected to Data Pin 14 of 74HC595


byte bitmap[8][7];

int numZones = sizeof(bitmap) / 8; // 7* One Zone refers to one 8 x 8 Matrix ( Group of 8 columns)
int maxZoneIndex = numZones - 1; // 6*
int numCols = numZones * 8; // 56*
...
void RefreshDisplay()
{
  for (int row = 0; row < 8; row++) {
    int rowbit = 1 << row;
    digitalWrite(rowLatchPin, LOW);//Hold latchPin LOW for transmitting data
    shiftOut(rowDataPin, rowClockPin, LSBFIRST, rowbit);   //Transmit data

    //Start sending column bytes
    digitalWrite(colLatchPin, LOW);//Hold latchPin LOW for transmitting data

    //Shift out to each matrix
    for (int zone = maxZoneIndex; zone >= 0; zone--)
    {
      shiftOut(colDataPin, colClockPin, MSBFIRST, bitmap[row][zone]);
    }

    //flip both latches at once to eliminate flicker
    digitalWrite(colLatchPin, HIGH);//Return the latch pin 1 high to signal chip
    digitalWrite(rowLatchPin, HIGH);//Return the latch pin 2 high to signal chip
...
// every row is fed with the column data
